...
Jalankan sekali secara manual untuk memastikan seluruh subsistem bekerja sebelum didaftarkan sebagai layanan otomatis.

---

## Auto-Boot (Systemd Service)

Agar program berjalan otomatis setiap Raspberry Pi dinyalakan tanpa perlu SSH terlebih dahulu, program didaftarkan sebagai *systemd service*. Metode ini dipilih karena paling andal: mendukung **auto-restart** bila aplikasi berhenti akibat *crash*, menunggu jaringan siap sebelum start, dan mencatat seluruh keluaran ke `journalctl`.

Berkas unit `timbangan-iot.service` sudah tersedia di root repositori. Periksa dan sesuaikan lebih dulu bila username atau lokasi folder proyek berbeda:

```bash
whoami                          # nama user — sesuaikan User= dan path /home/<user>/
pwd                             # pastikan path proyek cocok dengan WorkingDirectory=
nano timbangan-iot.service
```

Pastikan juga user tergabung dalam grup perangkat keras yang dipakai (perlu *logout* atau *reboot* agar berlaku):

```bash
sudo usermod -aG gpio,i2c,video $USER
```
...
**1.2 Reload systemd & enable auto-start**

```bash
sudo systemctl daemon-reload
sudo systemctl enable timbangan-iot.service
```

**1.3 Start service**

```bash
sudo systemctl start timbangan-iot.service
```

**1.4 Cek status**

```bash
sudo systemctl status timbangan-iot.service
```

Keluaran `active (running)` menandakan layanan berhasil berjalan. Untuk memantau log secara langsung (setara melihat terminal saat menjalankan manual):

```bash
journalctl -u timbangan-iot.service -f
```

### 2. Menonaktifkan Auto Boot (untuk troubleshooting)

Saat ingin melakukan *troubleshooting* atau perubahan kode, nonaktifkan auto boot agar service tidak berjalan otomatis dan tidak berebut akses perangkat keras dengan proses manual.

**2.1 Disable auto-start & stop service**

```bash
# Nonaktifkan auto-start saat boot
sudo systemctl disable timbangan-iot.service

# Stop service yang sedang berjalan
sudo systemctl stop timbangan-iot.service
```

**2.2 Verifikasi status**

```bash
sudo systemctl status timbangan-iot.service
```

Status `inactive (dead)` disertai keterangan `disabled` berarti layanan benar-benar berhenti dan tidak akan bangkit lagi saat *reboot*. Setelah selesai, aktifkan kembali dengan `enable` + `start`. Bila hanya mengubah kode Python tanpa mengubah berkas unit, cukup:

```bash
sudo systemctl restart timbangan-iot.service
```

Bila yang diubah adalah berkas `timbangan-iot.service` itu sendiri, salin ulang ke `/etc/systemd/system/` lalu jalankan `sudo systemctl daemon-reload` sebelum `restart`.

### 3. Catatan penting

| Hal | Keterangan |
|---|---|
| **Jangan jalankan ganda** | Selama service aktif, jangan menjalankan `python3 main.py` secara manual. Dua proses akan berebut bus I2C dan pin GPIO sehingga LCD menampilkan karakter acak. Jalankan `sudo systemctl stop` lebih dulu. |
| **Shutdown bersih** | Unit memakai `KillSignal=SIGINT` agar `systemctl stop` memicu jalur *shutdown* bertahap yang sama seperti `CTRL+C` (LCD → kamera → HX711 → GPIO), bukan pemutusan paksa. |
| **Kredensial** | `.env` tetap dibaca otomatis dari folder proyek, jadi Script ID tidak perlu ditulis di dalam berkas unit. |
| **Auto-restart** | `Restart=on-failure` dengan jeda 10 detik: layanan bangkit sendiri bila proses berhenti karena kesalahan, tetapi tidak terjebak *restart loop* saat berhenti normal. |
| **Log** | Semua keluaran masuk ke `journalctl`, sementara log aplikasi tetap ditulis ke folder `logs/` oleh `scripts/common.py`. |
